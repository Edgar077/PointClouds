...
namespace OpenTK.Extension
{
    ...
    public class Shader : IDisposable
    {
        ...
        /// </summary>
        public ShaderParam[] ShaderParams { get; private set; }
        ...
        {
            List<ShaderParam> shaderParams = new List<ShaderParam>();
            ...
            for (int i = 0; i < tokens.Count; i++)
            {
                if (tokens[i].TokenType == GlslLexer.TokenType.Keyword && (tokens[i].Text == "uniform" || tokens[i].Text == "attribute" || tokens[i].Text == "in"))
                {
                    ...
                    ShaderParamType paramType = (tokens[i].Text == "uniform" ? ShaderParamType.Uniform : ShaderParamType.Attribute);
                    ...
                    {
                        if (tokens[i].TokenType == GlslLexer.TokenType.Word) shaderParams.Add(new ShaderParam(type, paramType, tokens[i].Text));
