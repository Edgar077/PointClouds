    ...
    public class BitmapFont : IDisposable
    {
        ...
        public VAO CreateString(ShaderProgram Program, string Text)
        {
            ...
            for (int i = 0; i < Text.Length; i++)
            {
                ...
                indices[i * 6 + 0] = Convert.ToUInt32(i * 4 + 1);
                indices[i * 6 + 1] = Convert.ToUInt32(i * 4 + 0);
                indices[i * 6 + 2] = Convert.ToUInt32(i * 4 + 2);
                indices[i * 6 + 3] = Convert.ToUInt32(i * 4 + 1);
                indices[i * 6 + 4] = Convert.ToUInt32(i * 4 + 2);
                indices[i * 6 + 5] = Convert.ToUInt32(i * 4 + 3);
            }

            // Create the vertex buffer objects and then create the array object
            VBO<Vector3> vertexArray = new VBO<Vector3>("Font Vertices", vertices, BufferTarget.ArrayBuffer, BufferUsageHint.StaticRead);
            VBO<Vector2> uvArray = new VBO<Vector2>("Font UV", uvs, BufferTarget.ArrayBuffer, BufferUsageHint.StaticRead);
            VBO<uint> elementArray = new VBO<uint>("Font indices", indices, BufferTarget.ElementArrayBuffer, BufferUsageHint.StaticRead);
