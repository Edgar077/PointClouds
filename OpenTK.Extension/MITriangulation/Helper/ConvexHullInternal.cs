...
namespace MIConvexHull
{
    ...
    internal class ConvexHullInternal
    {
        ...
        int DetermineDimension()
        {
            ...
            for (var i = 0; i < 10; i++)
                dimensions.Add(InputVertices[r.Next(VCount)].Vertex.Count);
        ...
        /// <returns></returns>
        float GetSquaredDistanceSum(VertexWrap pivot, List<VertexWrap> initialPoints)
        {
            ...
            for (int i = 0; i < initPtsNum; i++)
            {
                ...
                for (int j = 0; j < Dimension; j++)
                {
                    float t = (initPt.PositionData[j] - pivot.PositionData[j]);
                    sum += t * t;
                }
            }
        ...
        private ConvexHullInternal(IEnumerable<IVector> pointCloud)
        {
            ...
            InputVertices = new List<VertexWrap>(pointCloud.Select((v, i) => new VertexWrap { Vertex = v, PositionData = v.ToArray<float>(), Index = i }));
